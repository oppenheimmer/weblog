...
// A Markdown backslash escape: the punctuation after it is literal text.
const ESCAPED = /\\([!-/:-@[-`{-~])/g;
const HELD = /(\d+)/g;
...
export function toPlainText(markdown) {
  return markdown
    .replace(/```[\s\S]*?```/g, " ")
    .replace(/`[^`]*`/g, " ")
    // Escaped punctuation is held aside before maths and markup are stripped,
    // then put back as written: "\$\$a+b\$\$" is text, not maths, and became
    // "\ a+b\" when the dollars were read as delimiters.
    .replace(ESCAPED, (_, mark) => `${mark.charCodeAt(0)}`)
    .replace(/\$\$[\s\S]*?\$\$/g, " ")
    .replace(/\$[^$]*\$/g, " ")
    .replace(/!\[[^\]]*\]\([^)]*\)/g, " ")
    .replace(/\[([^\]]*)\]\([^)]*\)/g, "$1")
    ...
    .replace(/\s+([,.;:!?])/g, "$1")
    .replace(/([,;:])(?:\s*\1)+/g, "$1")
    .replace(HELD, (_, code) => String.fromCharCode(Number(code)))
    .trim();
}
...
export function renderInteractive(content) {
  ...
  const common =
    `data-interactive="${payload.kind}" data-interactive-src="${attr(payload.src)}" ` +
    `data-interactive-name="${attr(payload.name)}"`;

  if (payload.kind === "figure") {
    // §3.6: the engine emits a unique root and the module mounts into it. The
    // root is an organization and styling boundary, **not** a security one —
    // a figure runs as page code by design, and nothing here pretends
    // otherwise. The id is derived from the bundle's own public path, so two
    // figures in one post cannot collide and lib/markup.mjs can namespace it
    // per post the way it namespaces every other id.
    const root = `figure-${payload.name}-${payload.src.split("/").slice(-2, -1)[0]}`;
    const deps = payload.dependencies.length
      ? ` data-interactive-deps="${attr(payload.dependencies.join(" "))}"`
      : "";
    return (
      `<figure class="interactive interactive--figure" ${common}${deps}>` +
      `<div class="interactive-root" id="${attr(root)}"></div>` +
  ...
  return (
    `<figure class="interactive" ${common}>` +
    `<div class="interactive-fallback">${payload.fallback}</div>` +
    `</figure>\n`
  );
}
