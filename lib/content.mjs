...
import { md, hasMath, readingTime, readingTimeFromText, toPlainText } from "./markdown.mjs";
...
export const RESERVED_SLUGS = new Set([
  "tags", "images", "assets", "styles", "downloads",
...
function renderBody(body, format) {
  return format === "latex" ? renderLatex(body) : md.render(body);
}
 ...
 */
export function normalizePost({ data, body, format, sourceName = "(unnamed)" }) {
  ...
  return {
    ...
    // Per-post embed hooks (see lib/templates.mjs postAssets). Only trusted,
    // engine-side content may set these; browser submissions never can.
...
export function loadPost(raw, { sourceName = "(unnamed)", format } = {}) {
