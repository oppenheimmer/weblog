...
import { renderLatex, stripHtml, LatexRenderError } from "./latex.mjs";
...
export function normalizePost({ data, body, format, sourceName = "(unnamed)", trust = DEFAULT_TRUST }) {
  ...
  let html;
  try {
    html = renderBody(body, format, trust);
  } catch (err) {
    // Too slow or too deep to render is the author's to fix, so it is reported
    // like any other content problem: in preview, as a publish refusal, and as
    // the build's named failure.
    if (err instanceof LatexRenderError) {
      throw new ContentError(`Post "${sourceName}": ${err.message}`, { source: sourceName, field: "body" });
    }
    throw err;
  }
