...
export const SITE = {
  title: "Sourav Mishra — Blog",
  author: "Sourav Mishra",
  role: "Senior Machine Learning Engineer",
  description:
    "Notes on computer vision, limited-data deep learning, LLMs, and applied AI by Sourav Mishra.",
...
// styles the page.
function shell({ title, description, canonical, math = false, head = "", bodyEnd = "", bodyClass = "", body }) {
  const katexCss = math
...
function tagChips(tags = [], wrapClass = "chip-row", role = "") {
  if (!tags.length) return "";
  // `role` is required in the listing table: a child of role="row" that carries
  // no cell role drops the whole tag column out of the accessibility tree.
  const roleAttr = role ? ` role="${role}"` : "";
  return `<div class="${wrapClass}"${roleAttr} aria-label="Tags">${tags
    .map((t) => `<a class="chip" href="/tags/${tagSlug(t)}/">${escapeHtml(t)}</a>`)
    .join("")}</div>`;
}

// Notion-style database table of posts, shared by the landing page and tag pages.
// Rows are <div>s (not <a>) so tag chips can be real links without nesting anchors;
// blog.js makes the whole row clickable via the title link.
function postTable(posts) {
  ...
  const rows = posts
    .map(
      (p) => `
        <div class="db-row" role="row">
          <span class="db-title" role="cell"><a href="/${p.slug}/">${escapeHtml(p.title)}</a></span>
          ${p.tags.length ? tagChips(p.tags, "db-tags", "cell") : `<span class="db-tags" role="cell"></span>`}
          <time class="db-date" role="cell" datetime="${p.date}">${formatDate(p.date)}</time>
        </div>`
    )
    .join("");
  return `<div class="post-db fade-up" role="table" aria-label="Articles">
          <div class="db-head" role="row">
...
export function listPage(posts) {
  const body = `
    <section class="section post-db-section" aria-label="Posts">
      <div class="shell">
        ${postTable(posts)}
      </div>
    </section>`;
...
export function tagPage(tag, posts) {
  ...
  const body = `
    <section class="section post-db-section" aria-label="Tagged posts">
      <div class="shell">
        <a class="back-link fade-up" href="/">
          <svg class="icon-svg" aria-hidden="true" viewBox="0 0 24 24"><use href="#icon-arrow-left"></use></svg>
          All posts
        ...
        <header class="tag-header fade-up">
          ...
          <p class="post-lede">${count} post${count === 1 ? "" : "s"} tagged &ldquo;${escapeHtml(tag)}&rdquo;.</p>
        </header>
        ${postTable(posts)}
      </div>
    </section>`;
  return shell({
    ...
    description: `All blog posts tagged "${tag}" by ${SITE.author}.`,
    canonical: `${SITE.url}/tags/${tagSlug(tag)}/`,
