...
import { KATEX_ASSET_ROOT } from "./katex-assets.mjs";
import { inlinesInFeed } from "./listing.mjs";
import { prepareArticle } from "./markup.mjs";
import { siteUrlFrom } from "./site.mjs";

export { DEFAULT_SITE_URL, siteUrlFrom } from "./site.mjs";
...
export const SITE = {
  ...
  // Read when used, never cached, so a process that sets SITE_URL after import
  // renders with it. No trailing slash.
  get url() {
    return siteUrlFrom(process.env.SITE_URL);
  },
        ...
        <div class="nav-bar">
          <a class="brand-copy" href="/" aria-label="Blog home">
            ...
            <span class="brand-text">
              <span class="brand-name">Weblog</span>
...
// These two tiny scripts have to run before the body is parsed. Naming their
// bodies lets the public-policy tripwire hash the exact emitted bytes and fail
// if vercel.json is not updated with them.
...
// experience.
const VIEW_SCRIPT_BODY = `(function(){var v=null;try{v=new URLSearchParams(location.search).get("view")}catch(e){}if(v!=="feed"&&v!=="table"){try{v=localStorage.getItem("${VIEW_STORAGE_KEY}")}catch(e){v=null}}if(v==="table")document.documentElement.setAttribute("data-view","table")})();`;
const VIEW_SCRIPT = `<script>${VIEW_SCRIPT_BODY}</script>`;
...
function shell({ title, description, canonical, math = false, head = "", bodyEnd = "", bodyClass = "", views = false, body }) {
  const katexCss = math
    ? `\n  <link rel="stylesheet" href="${KATEX_ASSET_ROOT}/katex.min.css" />`
    : "";
  return `<!DOCTYPE html>
<html lang="${SITE.lang}">
<head>
  ...
  <meta name="weblog-katex-css" content="${KATEX_ASSET_ROOT}/katex.min.css" />
  <script>${JS_CLASS_SCRIPT}</script>${views ? `\n  ${VIEW_SCRIPT}` : ""}
