...
export function createStagingPush(store, {
  ...
  async function prepare(staged, { postId: forced, replaceDraft = false } = {}) {
    ...
    for (const bundle of staged.bundles) {
      ...
      if (bundle.state === "complete") {
        ...
        const stored = id
          ...
          : null;
        // Publication resolves an interactive to its newest revision. An older
        // one pushed again lands on what is already stored and becomes newest
        // of nothing, so the site would keep the newer one while the push
        // reported success. Refused rather than pretended.
        const newest = stored?.status === "verified" ? records.get(id) : null;
        if (newest && newest.revisionId !== bundle.revisionId) {
          throw new StagingError(
            `${bundle.folder}/ is revision ${bundle.revisionId}, which this post stored before ` +
            `${newest.revisionId}. Publication uses an interactive's newest revision, so pushing an ` +
            "older one would not change the site; putting an earlier revision back is not supported.",
            { code: "older_revision" }
          );
        }
        bundles.push({ ...bundle, id, action: stored?.status === "verified" ? "stored" : "upload" });
        continue;
      }
      ...
      if (!target.draft) {
        ...
        if (sameContent(current, body)) {
          ...
          const expected = new Map(bundles.map((bundle) =>
            [bundle.id, bundle.action === "upload" ? bundle.revisionId : records.get(bundle.id)?.revisionId]));
    ...
    return { target, liveRevisionId, bundles, draftAction, body };
  }
  ...
  const api = {
    ...
    async push(staged, { log = () => {}, ...options } = {}) {
      ...
      for (const bundle of prepared.bundles) {
        ...
        log(`${bundle.kind.padEnd(8)} ${bundle.folder}/ ${agreed.unchanged ? "already stored as" : "uploaded as"} ${bundle.revisionId}`);
      }
      ...
      // 5. Confirm against R2, then delete what was confirmed.
