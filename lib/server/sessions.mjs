...
export function createSessionStore(store, {
  ...
  const api = {
     ...
     *
     * Expiry is enforced here on every read, independently of cleanup: a
     * record that outlives its own deadline must never authenticate, even
     * before the bucket's lifecycle rule deletes it (CLAUDE.md Step 9).
     */
    async verify(token) {
