...
/** The editor CSP, widened by exactly one origin when the page uploads. */
export function editorCsp({ upload = null } = {}) {
  return upload
    ? EDITOR_CSP.replace("connect-src 'self'", `connect-src 'self' ${upload}`)
    : EDITOR_CSP;
}

/** Headers every editor page sends. Only the editor itself talks to storage. */
export function pageHeaders({ uploads = false } = {}) {
  return {
    "content-type": "text/html; charset=utf-8",
    "cache-control": "no-store",
    "content-security-policy": editorCsp({ upload: uploads ? uploadOrigin() : null }),
