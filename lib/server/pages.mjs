...
import { distillStyleSources } from "../distill.mjs";
...
export function editorCsp({ upload = null, preview = false } = {}) {
  ...
  if (preview) {
    policy = policy
      ...
      // Plus the Distill template's own style blocks, by hash, as public pages
      // allow them (lib/distill.mjs): Run preview executes Distill figures, and a
      // srcdoc frame can only inherit this page's policy, never widen it.
      .replace("style-src 'self' https://fonts.googleapis.com",
        `style-src 'self' ${distillStyleSources()} https://fonts.googleapis.com; style-src-attr 'unsafe-inline'`);
  }
  return policy;
}
