  ...
  for (const revision of revisions) {
    const post = normalizePost({
      data: {
        ...
        tags: revision.tags,
        slug: revision.slug,
        ...(revision.math === undefined ? {} : { math: revision.math }),
      },
      body: revision.body,
      format: revision.format,
      sourceName: `${revision.slug} (${revision.revisionId})`,
      // Untrusted by omission *and* by intent: published revisions were written
      // through the editor, so raw HTML in a body is escaped rather than
      // rendered, and the per-post script hooks are unavailable to them.
    });
    ...
    posts.push({
      ...
      // What the build must fetch and verify for this post (lib/server/media-sync.mjs).
      media: revision.media ?? [],
      interactives: revision.interactives ?? [],
    });
  }
