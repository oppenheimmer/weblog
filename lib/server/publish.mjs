...
import {
  resolveInteractiveReferences, findInteractiveReferences, InteractiveError, publicBundlePath,
  ...
  const api = {
    ...
    async publish(draft, { idempotencyKey } = {}) {
      ...
      try {
        ...
        // Written once. A stored revision's time is when it first went out,
        // and the editor orders history by it; overwriting on every publish
        // moved an old revision to the top when it was put back. A stored copy
        // whose content no longer matches what publishing produces is
        // rewritten, since the build reads that copy.
        const key = revisionKey(draft.postId, draft.revisionId);
        try {
          await store.createJson(key, revision);
        } catch (err) {
          if (!(err instanceof ConflictError)) throw err;
          const stored = (await store.getJson(key))?.data;
          if (!stored || contentDigest(stored) !== digest) await store.putJson(key, revision);
        }
     ...
     */
    /**
     * Whether this draft revision went out with other bundle revisions than
     * its interactives resolve to now.
     *
     * A draft names interactives, not revisions, so replacing a bundle or
     * putting an earlier revision back changes what the post publishes without
     * changing the draft. Publishing that same revision again would be answered
     * with its old job, and the site's status is matched by revision id, so the
     * change needs a revision of its own. False for a revision never published:
     * publishing it resolves every interactive afresh.
     */
    async bundlesChangedSince(draft) {
      const published = (await store.getJson(keys.publishedRevision(draft.postId, draft.revisionId)))?.data;
      if (!published) return false;
      const referenced = new Set(findInteractiveReferences(draft.body, draft.format).map(({ id }) => id));
      if (!referenced.size) return false;
      const now = new Map((await interactiveStore.list(draft.postId)).map((record) => [record.id, record.revisionId]));
      const was = new Map((published.interactives ?? []).map((item) => [item.id, item.revisionId]));
      return [...referenced].some((id) => now.has(id) && was.get(id) !== now.get(id));
    },
