...
import { resolveInteractiveReferences, InteractiveError } from "../interactives.mjs";
...
export async function resolveForPublish(draft, uploads, bundles) {
  ...
  try {
    ...
    return { body: withBundles.body, media, interactives };
  } catch (err) {
...
export function createPublisher(store, {
  ...
  const api = {
    ...
    async publish(draft, { idempotencyKey } = {}) {
      ...
      const { body, media, interactives } = await resolveForPublish(draft, uploads, interactiveStore);
      ...
      try {
        ...
        for (const bundle of interactives) {
          for (const file of bundle.files) {
            await store.copy(
              keys.interactiveFile(draft.postId, bundle.id, bundle.revisionId, file.name),
              keys.publishedInteractive(draft.postId, bundle.id, bundle.revisionId, file.name)
            );
          }
        }
