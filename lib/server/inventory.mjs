  ...
  const post = (postId) => {
    if (!posts.has(postId)) {
      posts.set(postId, {
        ...
        revisions: [], media: [], uploads: [], bytes: 0,
      });
    }
...
export function collectableForPost(record, { now = Date.now(), retention = RETENTION } = {}) {
  ...
  if (record.state === "orphan") {
    return [...record.revisions, ...record.media, ...record.uploads].filter(old);
  }
  ...
  // Media of a live post is always kept: it is either in use, or in a revision
  // someone may roll back to.
  return doomed;
}
...
export function formatTree(inventory) {
  ...
  for (const post of inventory.posts) {
    ...
    for (const [label, items] of [["media", post.media], ["pending uploads", post.uploads]]) {
