...
export function collectableForPost(record, { now = Date.now(), retention = RETENTION } = {}) {
  ...
  // Media, attachments and interactive bundles of a live post are always kept:
  ...
  // is not. Deciding that needs to know which interactive revisions a stored
  // post revision froze, which means reading revision bodies rather than
  // listing keys — so it waits for the publication path that writes them
  // (§3.6, 6C). Keeping a bundle nobody references costs storage; deleting one
  // a rollback still needs breaks a published page.
  return doomed;
}
