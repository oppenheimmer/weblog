  ...
  const post = (postId) => {
    if (!posts.has(postId)) {
      posts.set(postId, {
        ...
        revisions: [], media: [], uploads: [], attachments: [], bytes: 0,
      });
    }
...
export function collectableForPost(record, { now = Date.now(), retention = RETENTION } = {}) {
  ...
  // Nothing references this post any more: the whole prefix goes.
  if (record.state === "orphan") {
    return [...record.revisions, ...record.media, ...record.uploads, ...record.attachments].filter(old);
  }
  ...
  // Media and attachments of a live post are always kept: they are in use, in a
...
export function formatTree(inventory) {
  ...
  for (const post of inventory.posts) {
    ...
    for (const [label, items] of [["media", post.media], ["attachments", files], ["pending uploads", post.uploads]]) {
