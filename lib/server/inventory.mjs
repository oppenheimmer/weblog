...
export async function buildInventory(store, { now = () => Date.now() } = {}) {
  ...
  for (const record of posts.values()) {
    ...
    const facts = { complete: true, newest: {}, verified: [], named: [] };
    try {
...
export function collectableForPost(record, { now = Date.now(), retention = RETENTION } = {}) {
  ...
  //
  //   * A private revision goes once it is neither what its interactive now
  //     resolves to nor named by any stored published revision, and has been
  //     stored for the draft-history window. Its manifest goes first, so a sweep
  //     that stops part way leaves loose files — which the interrupted-upload
  //     rule above collects — never a manifest claiming files that are gone.
  ...
  if (facts?.complete) {
    ...
    for (const [group, entry] of revisions) {
      if (!entry.manifest || !verified.has(group) || named.has(group)) continue;
      const [id, revisionId] = group.split("@");
      if (!facts.newest[id] || facts.newest[id] === revisionId) continue;
      if (!old(entry.manifest) || age(entry.manifest) < retention.draftRevisionMs) continue;
...
export async function collectableJobs(store, tree, { now = Date.now(), retention = RETENTION } = {}) {
  ...
  for (const job of tree.jobs ?? []) {
    ...
    if (age < retention.minAgeMs || age < retention.publicationJobMs) continue;
