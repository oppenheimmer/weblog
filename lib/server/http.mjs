...
import { requestRecord, writeLog } from "./log.mjs";
...
// Marks a browser that has signed in before, so it keeps a login budget of its
// own (lib/server/rate-limit.mjs). Same attributes as the session cookie.
const PROD_DEVICE_COOKIE = "__Host-weblog_device";
const DEV_DEVICE_COOKIE = "weblog_device_dev";

export const isProduction = () =>
  process.env.VERCEL_ENV === "production" || process.env.NODE_ENV === "production";

export const cookieName = () => (isProduction() ? PROD_COOKIE : DEV_COOKIE);
export const deviceCookieName = () => (isProduction() ? PROD_DEVICE_COOKIE : DEV_DEVICE_COOKIE);

export function serializeCookie(value, { maxAge, expire = false, name = cookieName() } = {}) {
  const parts = [
    `${name}=${expire ? "" : value}`,
    "Path=/",
    "HttpOnly",
...
export function readCookie(req, name = cookieName()) {
...
export function sendError(res, status, code, message, { fields, requestId = newRequestId() } = {}) {
  // The request log reads the code from here; the message never goes there.
  res.errorCode = code;
...
export function allowedOrigins() {
  ...
  return [...siteOrigins, process.env.EDITOR_ORIGIN]
    .filter(Boolean)
...
export function context() {
  ...
  cached = {
    ...
    // Signs Run preview grants (lib/server/run-grants.mjs), domain-separated there.
    runSecret: process.env.RATE_LIMIT_HASH_SECRET,
  };
  return cached;
}
...
export function route(handler, {
  ...
  return async function wrapped(req, res) {
    ...
    // What a handler adds to its line. Only known ids and names survive
    // (lib/server/log.mjs), whatever is passed.
    const fields = {};
    const annotate = (more) => Object.assign(fields, more);
    let failure = null;
    try {
      // HEAD is GET without a body, and Node suppresses the body itself. Deriving
      // it here means no route has to list it, and link checkers and proxies stop
      // getting a 405 for a perfectly ordinary request.
      const readsGet = methods.includes("GET");
      const allowed = readsGet && !methods.includes("HEAD") ? [...methods, "HEAD"] : methods;
      if (!allowed.includes(req.method)) {
        res.setHeader("allow", allowed.join(", "));
        return sendError(res, 405, "method_not_allowed", `${req.method} is not allowed here.`, { requestId });
      }

      // Present HEAD to the handler as GET. Handlers dispatch on req.method and
      // typically fall through to their mutating branch for anything else — so
      // without this, HEAD /api/drafts/ would create a draft. Normalizing here
      // makes that class of mistake impossible rather than something each
      // handler has to remember.
      if (req.method === "HEAD" && readsGet) req.method = "GET";
      ...
      return await handler({ req, res, session, requestId, annotate, ...context() });
    } catch (err) {
      failure = err;
      ...
      writeLog(requestRecord({
        requestId, method: req.method, url: req.url, status: res.statusCode || 200,
        ms: Date.now() - started, fields, error: failure, code: res.errorCode,
      }), cached?.log);
    }
  };
}
