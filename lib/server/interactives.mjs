...
import {
  validateManifest, computeRevisionId, validateFallback, InteractiveError,
...
/**
 * When a revision last became the one its interactive resolves to.
 *
 * A reference names an interactive, never a revision (§4.2), and publication
 * takes the most recent. Uploading makes a revision most recent; so does
 * putting an earlier one back, which records `promotedAt` on its manifest
 * rather than copying it, since a revision is its bytes and already exists.
 */
export const revisionOrder = (record) => String(record.promotedAt ?? record.createdAt ?? "");
...
export function createInteractives(store, {
  ...
  // Revisions are ordered by these times, so two stamped in the same
  // millisecond would have no order at all. Strictly increasing per store.
  let lastStamp = 0;
  const stamp = () => {
    const time = Math.max(now().getTime(), lastStamp + 1);
    lastStamp = time;
  ...
  const api = {
    /** Every verified bundle revision of a post, least recently current first. */
    async list(postId) {
      assertPostId(postId);
      ...
      return records
        ...
        .sort((a, b) => revisionOrder(a).localeCompare(revisionOrder(b)) || a.id.localeCompare(b.id));
    },
    ...
    async begin({ postId, manifest, interactiveId } = {}) {
      ...
      // An earlier revision sent again is put back: it becomes the one this
      // interactive resolves to, which is what sending it means.
      const existing = (await store.getJson(keys.interactiveManifest(postId, id, revisionId)))?.data;
      if (existing?.status === "verified") {
        const current = await api.get(postId, id);
        if (current && current.revisionId !== revisionId) {
          const record = await api.promote({ postId, interactiveId: id, revisionId });
          return { interactiveId: id, revisionId, unchanged: true, promoted: true, uploads: [], record };
        }
        return { interactiveId: id, revisionId, unchanged: true, promoted: false, uploads: [], record: existing };
      }

      const bundles = await api.list(postId);
      // An interactive keeps its kind across revisions. Its references say
      // ::demo or ::figure, so a new revision of the other kind would turn
      // every one of them into a refusal at publish time.
      const prior = bundles.filter((record) => record.id === id).at(-1);
      if (prior && prior.kind !== validated.kind) {
        throw new BundleError(`${prior.name} is a ${prior.kind === "demo" ? "lab" : "figure"}; ` +
          `a new revision of it must be one too.`, { code: "wrong_kind", status: 409, field: "kind" });
      }
    ...
    async complete({ postId, uploadId } = {}) {
      ...
      // Publication validates the fallback, and refusing it there left a
      // bundle attached that could never publish. Its bytes are verified now,
      // so the refusal can be made while the author is still attaching.
      const fallback = verified.find(({ file }) => file.name === manifest.fallback);
      try {
        const text = new TextDecoder("utf-8", { fatal: true }).decode(fallback.bytes);
        validateFallback(text, { files: manifest.files, publicPath: "/" });
      } catch (err) {
        if (err instanceof InteractiveError) throw asBundleError(err);
        if (err instanceof TypeError) {
          throw new BundleError(`"${manifest.fallback}" is not UTF-8 text.`,
            { code: "fallback_refused", status: 422, field: "fallback" });
        }
        throw err;
      }
    ...
    /**
     * Put a stored revision back as the one its interactive resolves to.
     *
     * Only a verified revision of this post's interactive. The manifest is
     * rewritten conditionally with `promotedAt`, so two promotions racing each
     * other both land on a record that exists, and the later one wins. Takes
     * effect on the site at the next publish, like any bundle change.
     */
    async promote({ postId, interactiveId, revisionId } = {}) {
      assertPostId(postId);
      if (typeof interactiveId !== "string" || !INTERACTIVE_ID_PATTERN.test(interactiveId)) {
        throw new BundleError("Unknown interactive.", { code: "invalid_interactive", field: "interactiveId" });
      }
      if (typeof revisionId !== "string" || !INTERACTIVE_REVISION_PATTERN.test(revisionId)) {
        throw new BundleError("Unknown revision.", { code: "invalid_revision", field: "revisionId" });
      }
      const key = keys.interactiveManifest(postId, interactiveId, revisionId);
      const stored = (await store.getJson(key))?.data;
      if (stored?.status !== "verified" || stored.postId !== postId) {
        throw new BundleError("That revision of this interactive is not stored any more.", {
          code: "not_found", status: 404, field: "revisionId",
        });
      }
      const current = await api.get(postId, interactiveId);
      if (current?.revisionId === revisionId) return current;
      const { __etag, ...record } = await store.mutateJson(key, (data) =>
        data?.status === "verified" ? { ...data, promotedAt: stamp() } : undefined);
      return record;
    },
