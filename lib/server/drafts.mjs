...
/**
 * Revision identity, time-ordered so revisions sort chronologically by key
 * alone. That makes "the most recent few revisions" a listing concern rather
 * than something requiring every object to be read.
 */
export function newRevisionId(now = Date.now()) {
  return `r_${now.toString(36).padStart(9, "0")}_${crypto.randomBytes(4).toString("hex")}`;
}
