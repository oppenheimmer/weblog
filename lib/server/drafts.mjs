...
const MAX_BODY_BYTES = 1_000_000; // 1 MB; expansion has a separate 1 MiB ceiling
...
export function createDraftStore(store, { now = () => new Date().toISOString() } = {}) {
  ...
  const api = {
     ...
     *
     * `pushedDigest` is for the staging push only, and deliberately an option
     * rather than an input field, so a browser request body can never set it
     * (lib/server/staging.mjs).
     */
    async create(input = {}, { pushedDigest } = {}) {
     ...
     *
     * `pushedDigest` records what a staging push wrote. It survives later saves
     * unchanged, which is what makes it useful: a browser edit leaves the digest
     * describing content the draft no longer has.
     */
    async save(postId, input, etag, { pushedDigest } = {}) {
      assertPostId(postId);
      if (!etag) {
    ...
    async remove(postId) {
      ...
      const { deletePostObjects, deletePostJobs } = await import("./inventory.mjs");
      const removed = await deletePostObjects(store, postId, { apply: true });
      // Its publication jobs name it but live outside its prefixes. The index
      // no longer names the post, so none can answer a publish again, and a
      // discarded post leaves nothing behind.
      await deletePostJobs(store, postId);
