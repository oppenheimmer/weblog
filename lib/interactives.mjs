 ...
 */
export function validateFallback(html, { files = [], publicPath = "" } = {}) {
...
export function resolveInteractiveReferences(body, {
  ...
  let out = replaceOutsideCode(source, "markdown", MD_FIGURE, (match) => {
    // The bundle contract already accepts figures, so the id resolves; what is
    // missing is the page-side grant that runs one (§3.6, 6D). Refusing here
    // beats publishing a directive that would render as literal text.
    lookup(match[1], "figure");
    fail("Article figures are not available yet; only ::demo is.",
      { code: "figure_unavailable", field: "body" });
  });

  out = replaceOutsideCode(out, "markdown", MD_DEMO, (match) => {
    const record = lookup(match[1], "demo");
    used.add(record.id);
...
export function readInteractivePayload(text) {
  ...
  if (payload.kind !== "demo") return null;
  if (typeof payload.src !== "string") return null;
  if (!/^\/demos\/[a-z0-9][a-z0-9-]*\/[a-z0-9][a-z0-9-]*\/iv_[0-9a-f]{16}\/[A-Za-z0-9][A-Za-z0-9._/-]*\.html$/
    .test(payload.src)) {
    return null;
  }
  try {
