...
/** The ordinary `source` shape: literal segments, with `(.*)` as the wildcard. */
const SUPPORTED_SOURCE = /^\/(?:[A-Za-z0-9\-._~/]|\(\.\*\))*$/;
// The public-page CSP needs one path-to-regexp negative lookahead so it cannot
// land on editor, login, API or bundle documents. Keep that exceptional shape
// exact: broad regex support here would make the local model look more capable
// than it is.
const PUBLIC_PAGE_SOURCE =
  "/((?!api(?:/|$)|editor(?:/|$)|login(?:/|$)|demos(?:/|$)|assets/figures(?:/|$)).*)";
...
export function compileSource(source) {
  if (source === PUBLIC_PAGE_SOURCE) {
    return /^\/(?!api(?:\/|$)|editor(?:\/|$)|login(?:\/|$)|demos(?:\/|$)|assets\/figures(?:\/|$)).*$/;
  }
...
export function readRouting(config) {
  ...
  // Rewrites matter to the emitted tree rather than to headers: a static file
  // at a rewritten address would shadow the function that address exists for.
